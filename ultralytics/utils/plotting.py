...
class Annotator:
    ...
    def display_analytics(self, im0, text, txt_color, bg_color, margin, text_offset):
        """
        ...
        text_y_offset = 0 + text_offset[1]
        text_x_offset = text_offset[0]
        for label, value in text.items():
            txt = f"{label}: {value}"
            text_size = cv2.getTextSize(txt, 0, self.sf, self.tf)[0]
            if text_size[0] < 5 or text_size[1] < 5:
                text_size = (5, 5)
            text_x = text_x_offset+ im0.shape[1] - text_size[0] - margin * 2 - horizontal_gap
